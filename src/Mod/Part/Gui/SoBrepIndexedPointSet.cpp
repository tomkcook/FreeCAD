...
#include "SoBrepIndexedPointSet.h"
#include <Gui/SoFCUnifiedSelection.h>
#include <Gui/SoFCSelectionAction.h>

using namespace PartGui;

SO_NODE_SOURCE(SoBrepIndexedPointSet)

void SoBrepIndexedPointSet::initClass()
{
    SO_NODE_INIT_CLASS(SoBrepIndexedPointSet, SoIndexedPointSet, "IndexedPointSet");
}

SoBrepIndexedPointSet::SoBrepIndexedPointSet()
    :selContext(std::make_shared<SelContext>())
    ,selContext2(std::make_shared<SelContext>())
{
    SO_NODE_CONSTRUCTOR(SoBrepIndexedPointSet);
}

void SoBrepIndexedPointSet::GLRender(SoGLRenderAction *action)
{
...
void SoBrepIndexedPointSet::GLRenderBelowPath(SoGLRenderAction * action)
{
    inherited::GLRenderBelowPath(action);
}

void SoBrepIndexedPointSet::getBoundingBox(SoGetBoundingBoxAction * action) {
    ...
    SbBox3f bbox;
    for(auto coordIdx : ctx2->selectionIndex) {
        auto idx = this->coordIndex[coordIdx];
...
void SoBrepIndexedPointSet::renderHighlight(SoGLRenderAction *action, SelContextPtr ctx)
{
    ...
    if(coords3d) {
        if(id == INT_MAX) {
            ...
            glEnd();
        } else if (id < 0 || id >= this->coordIndex.getNum()) {
...
void SoBrepIndexedPointSet::renderSelection(SoGLRenderAction *action, SelContextPtr ctx, bool push)
{
    SoState * state = action->getState();
    if(push) {
        state->push();
    ...
    if(coords3d) {
        ...
        if(ctx->isSelectAll()) {
            ...
            for(auto idx : ctx->selectionIndex) {
                if(idx >= 0 && idx < this->coordIndex.getNum())
                    glVertex3fv((const GLfloat*) (coords3d + this->coordIndex[idx]));
                else
                    warn = true;
            }
        }
    ...
    if(warn)
        SoDebugError::postWarning("SoBrepIndexedPointSet::renderSelection", "selectionIndex out of range");
    if(push) state->pop();
}

void SoBrepIndexedPointSet::doAction(SoAction* action)
{
